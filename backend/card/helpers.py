...
def get_editable_card_queryset_by_request(request: rest_framework.request.Request) -> QuerySet:
    collection_ids = collection_helpers.get_editable_collection_queryset_by_request(
        request).values_list("id", flat=True)
    return Card.objects.filter(collection_id__in=collection_ids)
